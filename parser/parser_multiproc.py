...
def completion():
    ...
    if len(glob.glob("*.db")) > 1:
        os.remove("price.db")
        os.rename("new-price.db", "price.db")
    else:
        pass
...
def writing_data_to_database(table_name, departure, appointment, ak, cost):
    ...
    cur = con.cursor()
    cur.execute(f"INSERT INTO {table_name} VALUES (?, ?, ?, ?, ?)", (id, departure, appointment, ak, cost))
    con.commit()
    con.close()
...
if __name__ == "__main__":
    cleaning_directory()
    check_db()
    parsers = [scraping_transcomavia, scraping_artis, scraping_md_cargo, aerosib, aerodar]
    procs = []
    for parser in parsers:
        proc = Process(target=parser, args=())
        procs.append(proc)
        proc.start()
